...
module.exports = {
    ...
    async createAtividadeSenai(request, response) {
        try {
            ...
            return response.json({ error: erros.message })
>>>>>>> 860ddd3eb8938dc05f36d4f3c0d4865a90607b9f
        }
      }
      console.log(hoursPerActivity)
      //Testado -- OK
  ...
  async createAtividadeSenai(request, response) {
    try {
      ...
      if (!(await validator.isValid(request.body))) {
        return response
          .status(400)
          .json({ error: 'Nome da Atividade é campo obrigatório.' })
      }
    ...
    try {
      ...
      if (idstatus === 2) {
        return response.status(400).json({
          error: 'Esta atividade já está aguardando aprovação do coordenador.',
        })
      } else {
        if (senaiEvent === 1) {
          if (idactivity === null || idcategory === null) {
